...
function dfs(grid: string[][], keypad: Keypad, code: string, robot: number): number {
    let current = "A";
    let length = 0;
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (robot === 0) {
            ...
            length += Math.min(
                ...moves.map((move) =>
                    dfs(directionalKeypadGrid, directionalKeypad, move, robot - 1)
                )
            );
        }
...
for (const code of codes) {
    sum += parseInt(code) * dfs(numericKeypadGrid, numericKeypad, code, 2);
}
